...
{
    pub app_exit_trigger: Option<InputDeviceConfig>,
    pub ui_outline_trigger: Option<InputDeviceConfig>,
}
    ...
    fn build(&self, _application: &mut App)
    {
        ...
        if let Some(app_exit_trigger) = self.app_exit_trigger
        {
            _application
                ...
                .add_systems(
                    Update,
                    send_exit_app_message, // .in_set(execution_order::Before::HitAnyKey)
                );
        }
        ...
        if let Some(ui_outline_trigger) = self.ui_outline_trigger
        {
            _application
                ...
                .add_systems(
                    Update,
                    toggle_ui_outline_gizmo, // .in_set(execution_order::Before::HitAnyKey)
                );
        }
    }
}
...
{
    // キー／ボタンが押下されたなら
    if controller.is_pressed_with_reset(&config.0)
    {
        // アプリ終了メッセージを送信
        message_exit_app.write(AppExit::Success);
    }
}
